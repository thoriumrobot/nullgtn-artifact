...
public class TemperatureCardView extends CardView implements BaseColdsnapView<GraphCardViewModel>, FeedCardView {
    ...
    @BindView(R.id.xyp_temperature)
    XYPlot xypTemperature;
    ...
    private void addDataSeries(GraphCardViewModel vm) {
        vm.getDataSeries().forEach(dataSeries -> xypTemperature.addSeries(mapDataSeriesToGraphSeriesImpl(dataSeries), mapDataSeriesToLineAndPointFormatter(dataSeries)));
    }

    private SimpleXYSeries mapDataSeriesToGraphSeriesImpl(DataSeries dataSeries) {
        return new SimpleXYSeries(dataSeries.getData().stream().map(DataValue::getXValue).collect(Collectors.toList()), dataSeries.getData().stream().map(DataValue::getYValue).collect(Collectors.toList()), "Title");
    }
    ...
    protected void drawLine(GraphCardViewModel vm, GraphLine graphLine) {
        ...
        yVals.add(graphLine.getRangeValue());
        xypTemperature.addSeries(new SimpleXYSeries(xVals, yVals, getContext().getString(R.string.cold_threshold)), new LineAndPointFormatter(graphLine.getLineColor(), null, null, null));
    }
}
