...
public class ColdAlarm extends BroadcastReceiver implements ColdAlarmView {
    ...
    @Inject
    ColdAlarmPresenter coldAlarmPresenter;
    ...
    public void onReceive(final Context context, final Intent intent) {
        ((com.yoxjames.coldsnap.ColdSnapApplication) context.getApplicationContext()).getInjector().coldAlarmSubcomponent(new ColdAlarmModule(this)).inject(this);
    ...
    public static void setAlarm(Context context, CSPreferences csPreferences) {
        ...
        am.setInexactRepeating(AlarmManager.RTC, alarmTime.getTimeInMillis(), AlarmManager.INTERVAL_DAY, pi);
    }
    ...
    private void displayThresholdMessage(Context context, String tonightFormatted, String thresholdFormatted) {
        final int mId = 1;
        NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(context).//.setSmallIcon(R.drawable.cold_alarm_icon)
        setContentTitle("ColdSnap: Cold Warning").setContentText("Tonight's low " + tonightFormatted + " is at or below threshold " + thresholdFormatted).setColor(Color.BLUE);
    ...
    private void displayPlantMessage(Context context, String plantName) {
        final int mId = 2;
        NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(context).setSmallIcon(R.drawable.cold_alert).setContentTitle("ColdSnap: Plant in Danger!").setContentText(plantName + "is in danger of dying due to cold!").setColor(Color.BLUE);
        // Creates an explicit intent for an Activity in your app
        Intent resultIntent = new Intent(context, PlantListActivity.class);
        // The stack builder object will contain an artificial back stack for the
        // started Activity.
