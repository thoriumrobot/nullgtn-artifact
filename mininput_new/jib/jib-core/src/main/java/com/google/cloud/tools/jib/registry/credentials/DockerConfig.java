...
class DockerConfig {
    ...
    private static <K, T> Map.Entry<K, T> findFirstInMapByKey(Map<K, T> map, List<Predicate<K>> keyMatches) {
        return keyMatches.stream().map(keyMatch -> findFirstInMapByKey(map, keyMatch)).filter(Objects::nonNull).findFirst().orElse(null);
    }
    ...
    DockerCredentialHelper getCredentialHelperFor(String registry) {
        ...
        Map.Entry<String, ?> firstMatchInAuths = findFirstInMapByKey(dockerConfigTemplate.getAuths(), registryMatchers);
        if (dockerConfigTemplate.getCredsStore() != null && firstMatchInAuths != null) {
            return new DockerCredentialHelper(firstMatchInAuths.getKey(), dockerConfigTemplate.getCredsStore());
        }
        Map.Entry<String, String> firstMatchInCredHelpers = findFirstInMapByKey(dockerConfigTemplate.getCredHelpers(), registryMatchers);
        if (firstMatchInCredHelpers == null) {
            return null;
        }
        return new DockerCredentialHelper(firstMatchInCredHelpers.getKey(), firstMatchInCredHelpers.getValue());
    }
