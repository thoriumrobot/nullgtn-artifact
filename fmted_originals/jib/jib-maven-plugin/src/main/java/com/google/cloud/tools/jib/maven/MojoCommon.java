...
class MojoCommon {
    ...
    static AbsoluteUnixPath getAppRootChecked(JibPluginConfiguration jibPluginConfiguration) throws InvalidAppRootException {
        ...
        if (appRoot.isEmpty()) {
            ...
            appRoot = isWarProject ? JavaLayerConfigurations.DEFAULT_WEB_APP_ROOT : JavaLayerConfigurations.DEFAULT_APP_ROOT;
        }
        try {
    ...
    static Path getExtraDirectoryPath(JibPluginConfiguration jibPluginConfiguration) {
        return jibPluginConfiguration.getExtraDirectoryPath().orElse(Preconditions.checkNotNull(jibPluginConfiguration.getProject()).getBasedir().toPath().resolve("src").resolve("main").resolve("jib"));
    }
     ...
     */
    @VisibleForTesting
    static Map<AbsoluteUnixPath, FilePermissions> convertPermissionsList(List<PermissionConfiguration> permissionList) {
