...
public class FilesMojo extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        ImmutableSet.copyOf(project.getBuild().getResources()).stream().map(FileSet::getDirectory).forEach(System.out::println);
        ...
        Set<String> projectArtifacts = projects.stream().map(MavenProject::getArtifact).map(Artifact::toString).collect(Collectors.toSet());
        ...
        try {
            DependencyResolutionResult resolutionResult = projectDependenciesResolver.resolve(new DefaultDependencyResolutionRequest(project, session.getRepositorySession()).setResolutionFilter(ignoreProjectDependenciesFilter));
            resolutionResult.getDependencies().stream().map(org.eclipse.aether.graph.Dependency::getArtifact).filter(org.eclipse.aether.artifact.Artifact::isSnapshot).map(org.eclipse.aether.artifact.Artifact::getFile).forEach(System.out::println);
    ...
    private Path resolveExtraDirectory() {
        if (extraDirectory.getPath() == null) {
            return Preconditions.checkNotNull(project).getBasedir().getAbsoluteFile().toPath().resolve("src").resolve("main").resolve("jib");
        }
