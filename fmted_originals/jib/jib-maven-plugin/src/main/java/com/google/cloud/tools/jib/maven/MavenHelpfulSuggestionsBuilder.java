...
class MavenHelpfulSuggestionsBuilder {
    ...
    HelpfulSuggestions build() {
        boolean isCredHelperDefinedForBaseImage = jibPluginConfiguration.getTargetImageCredentialHelperName() != null;
        boolean isCredHelperDefinedForTargetImage = jibPluginConfiguration.getTargetImageCredentialHelperName() != null;
        return new HelpfulSuggestions(messagePrefix, "mvn clean", baseImageReference, !isCredHelperDefinedForBaseImage && !baseImageHasConfiguredCredentials, "<from><credHelper>", AUTH_CONFIGURATION_SUGGESTION, targetImageReference, !isCredHelperDefinedForTargetImage && !targetImageHasConfiguredCredentials, "<to><credHelper>", AUTH_CONFIGURATION_SUGGESTION, "<to><image>", "-Dimage", "pom.xml");
    }
}
