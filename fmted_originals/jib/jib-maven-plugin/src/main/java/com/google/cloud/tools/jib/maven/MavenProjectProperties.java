...
public class MavenProjectProperties implements ProjectProperties {
    ...
    static MavenProjectProperties getForProject(MavenProject project, Log log, Path extraDirectory, Map<AbsoluteUnixPath, FilePermissions> permissions, AbsoluteUnixPath appRoot) throws MojoExecutionException {
        try {
            return new MavenProjectProperties(project, makeEventHandlers(log), MavenLayerConfigurations.getForProject(project, extraDirectory, permissions, appRoot));
    ...
    private static EventHandlers makeEventHandlers(Log log) {
        return new EventHandlers().add(JibEventType.LOGGING, new LogEventHandler(log)).add(JibEventType.TIMING, new TimerEventHandler(log::debug));
    }
    ...
    @VisibleForTesting
    MavenProjectProperties(MavenProject project, EventHandlers eventHandlers, JavaLayerConfigurations javaLayerConfigurations) {
