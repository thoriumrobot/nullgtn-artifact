...
public class JibPlugin implements Plugin<Project> {
    ...
    static List<Project> getProjectDependencies(Project project) {
        return project.getConfigurations().stream().map(Configuration::getDependencies).flatMap(DependencySet::stream).filter(ProjectDependency.class::isInstance).map(ProjectDependency.class::cast).map(ProjectDependency::getDependencyProject).collect(Collectors.toList());
    }
    ...
    public void apply(Project project) {
        checkGradleVersion();
        JibExtension jibExtension = project.getExtensions().create(JIB_EXTENSION_NAME, JibExtension.class, project);
        Task buildImageTask = project.getTasks().create(BUILD_IMAGE_TASK_NAME, BuildImageTask.class).setJibExtension(jibExtension);
        Task buildDockerTask = project.getTasks().create(BUILD_DOCKER_TASK_NAME, BuildDockerTask.class).setJibExtension(jibExtension);
        Task buildTarTask = project.getTasks().create(BUILD_TAR_TASK_NAME, BuildTarTask.class).setJibExtension(jibExtension);
        Task filesTask = project.getTasks().create(FILES_TASK_NAME, FilesTask.class).setJibExtension(jibExtension);
        project.afterEvaluate(projectAfterEvaluation -> {
            try {
                War warTask = GradleProjectProperties.getWarTask(project);
                Task dependsOnTask;
                if (warTask != null) {
                    ExplodedWarTask explodedWarTask = (ExplodedWarTask) project.getTasks().create(EXPLODED_WAR_TASK_NAME, ExplodedWarTask.class).dependsOn(warTask);
                ...
                for (Project dependencyProject : computedDependencies) {
                    dependencyProject.getPlugins().withType(BasePlugin.class, unused -> {
                        Task assembleTask = dependencyProject.getTasks().getByPath(BasePlugin.ASSEMBLE_TASK_NAME);
                ...
                throw new GradleException("Could not find task 'classes' on project " + projectAfterEvaluation.getDisplayName() + " - perhaps you did not apply the 'java' plugin?", ex);
            }
        });
    }
}
