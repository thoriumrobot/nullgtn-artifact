...
public class MainVerticle extends AbstractVerticle {
    ...
    public void start(Future<Void> startFuture) {
        ...
        vertx.createHttpServer().requestHandler(router).listen(8080, asyncStart -> {
    ...
    private void hello(RoutingContext context) {
        ...
        context.response().putHeader("Content-Type", "text/plain").end("Hello from Vert.x!");
    }

    private void now(RoutingContext context) {
        logger.info("Time request from {}", context.request().remoteAddress());
        JsonObject data = new JsonObject().put("powered-by", "vertx").put("current-time", System.currentTimeMillis());
        context.response().putHeader("Content-Type", "application/json").end(data.encode());
    }
}
