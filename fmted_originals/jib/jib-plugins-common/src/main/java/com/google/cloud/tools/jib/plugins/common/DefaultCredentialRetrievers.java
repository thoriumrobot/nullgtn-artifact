...
public class DefaultCredentialRetrievers {
    ...
    public DefaultCredentialRetrievers setInferredCredential(Credential inferredCredential, String credentialSource) {
        inferredCredentialRetriever = credentialRetrieverFactory.known(inferredCredential, credentialSource);
        return this;
    }
        ...
        if (credentialHelper != null) {
            ...
            if (credentialHelper.contains(FileSystems.getDefault().getSeparator())) {
                if (Files.exists(Paths.get(credentialHelper))) {
                    credentialRetrievers.add(credentialRetrieverFactory.dockerCredentialHelper(credentialHelper));
                } else {
                    throw new FileNotFoundException("Specified credential helper was not found: " + credentialHelper);
                }
            } else {
                credentialRetrievers.add(credentialRetrieverFactory.dockerCredentialHelper(DockerCredentialHelper.CREDENTIAL_HELPER_PREFIX + credentialHelper));
            }
        }
