...
public class MainClassResolver {
    ...
    private static String findMainClassInClassFiles(ProjectProperties projectProperties) throws MainClassInferenceException {
        ...
        ImmutableList<Path> classesSourceFiles = projectProperties.getJavaLayerConfigurations().getClassLayerEntries().stream().map(LayerEntry::getSourceFile).collect(ImmutableList.toImmutableList());
        MainClassFinder.Result mainClassFinderResult = new MainClassFinder(classesSourceFiles, new DefaultEventDispatcher(projectProperties.getEventHandlers())).find();
        switch(mainClassFinderResult.getType()) {
            ...
            case MULTIPLE_MAIN_CLASSES:
                throw new MainClassInferenceException(HelpfulSuggestions.forMainClassNotFound("Multiple valid main classes were found: " + String.join(", ", mainClassFinderResult.getFoundMainClasses()), projectProperties.getPluginName()));
            default:
