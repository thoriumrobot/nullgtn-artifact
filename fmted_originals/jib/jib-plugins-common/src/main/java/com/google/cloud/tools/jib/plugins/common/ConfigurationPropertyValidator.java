...
public class ConfigurationPropertyValidator {
     ...
     */
    public static Optional<Credential> getImageCredential(EventDispatcher eventDispatcher, String usernameProperty, String passwordProperty, AuthProperty auth, String authUsernameDescriptor, String authPasswordDescriptor) {
        ...
        if (Strings.isNullOrEmpty(auth.getUsername())) {
            eventDispatcher.dispatch(LogEvent.warn(authUsernameDescriptor + " is missing from build configuration; ignoring auth section."));
            return Optional.empty();
        }
        if (Strings.isNullOrEmpty(auth.getPassword())) {
            eventDispatcher.dispatch(LogEvent.warn(authPasswordDescriptor + " is missing from build configuration; ignoring auth section."));
            return Optional.empty();
        }
