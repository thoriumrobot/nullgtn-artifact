...
public class HelpfulSuggestions {
    ...
    public static String forToNotConfigured(String messagePrefix, String parameter, String buildConfigFilename, String command) {
        return suggest(messagePrefix, "add a " + parameter + " configuration parameter to your " + buildConfigFilename + " or set the parameter via the commandline (e.g. '" + command + "').");
    }
    ...
    public String forCacheDirectoryNotOwned(Path cacheDirectory) {
        return suggest("check that '" + cacheDirectory + "' is not used by another application or set the `useOnlyProjectCache` " + "configuration");
    }
    ...
    public String forNoCredentialsDefined(String registry, String repository) {
        ...
        if (noCredentialsDefinedForBaseImage && registry.equals(baseImageReference.getRegistry()) && repository.equals(baseImageReference.getRepository())) {
            return forNoCredentialHelpersDefined(baseImageCredHelperConfiguration, baseImageAuthConfiguration.apply(registry));
        }
        if (noCredentialsDefinedForTargetImage && registry.equals(targetImageReference.getRegistry()) && repository.equals(targetImageReference.getRepository())) {
            return forNoCredentialHelpersDefined(targetImageCredHelperConfiguration, targetImageAuthConfiguration.apply(registry));
        }
    ...
    public String forGeneratedTag(String projectName, String projectVersion) {
        return "Tagging image with generated image reference " + projectName + ":" + projectVersion + ". If you'd like to specify a different tag, you can set the " + toImageConfiguration + " parameter in your " + buildConfigurationFilename + ", or use the " + toImageFlag + "=<MY IMAGE> commandline flag.";
    }
    ...
    private String forNoCredentialHelpersDefined(String credHelperConfiguration, String authConfiguration) {
        return suggest("set a credential helper name with the configuration '" + credHelperConfiguration + "' or " + authConfiguration);
    }
