...
class RegistryEndpointCaller<T> {
     ...
     */
    RegistryEndpointCaller(EventDispatcher eventDispatcher, String userAgent, String apiRouteBase, RegistryEndpointProvider<T> registryEndpointProvider, @Nullable Authorization authorization, RegistryEndpointRequestProperties registryEndpointRequestProperties, boolean allowInsecureRegistries) throws MalformedURLException {
        this(eventDispatcher, userAgent, apiRouteBase, registryEndpointProvider, authorization, registryEndpointRequestProperties, allowInsecureRegistries, Connection.getConnectionFactory(), null);
    }
    ...
    RegistryEndpointCaller(EventDispatcher eventDispatcher, String userAgent, String apiRouteBase, RegistryEndpointProvider<T> registryEndpointProvider, @Nullable Authorization authorization, RegistryEndpointRequestProperties registryEndpointRequestProperties, boolean allowInsecureRegistries, Function<URL, Connection> connectionFactory, @Nullable Function<URL, Connection> insecureConnectionFactory) throws MalformedURLException {
        ...
        this.initialRequestUrl = registryEndpointProvider.getApiRoute(DEFAULT_PROTOCOL + "://" + apiRouteBase);
    ...
    private T handleUnverifiableServerException(URL url) throws IOException, RegistryException {
        ...
        try {
            eventDispatcher.dispatch(LogEvent.info("Cannot verify server at " + url + ". Attempting again with no TLS verification."));
    ...
    private T fallBackToHttp(URL url) throws IOException, RegistryException {
        ...
        eventDispatcher.dispatch(LogEvent.info("Failed to connect to " + url + " over HTTPS. Attempting again with HTTP: " + httpUrl));
    ...
    private T call(URL url, Function<URL, Connection> connectionFactory) throws IOException, RegistryException {
        ...
        boolean sendCredentials = isHttpsProtocol(url) || JibSystemProperties.isSendCredentialsOverHttpEnabled();
        try (Connection connection = connectionFactory.apply(url)) {
            Request.Builder requestBuilder = Request.builder().setUserAgent(userAgent).setHttpTimeout(JibSystemProperties.getHttpTimeout()).setAccept(registryEndpointProvider.getAccept()).setBody(registryEndpointProvider.getContent());
            if (sendCredentials) {
