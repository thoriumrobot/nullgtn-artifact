...
public class ImageToJsonTranslator {
    ...
    static ImmutableList<String> environmentMapToList(@Nullable Map<String, String> environment) {
        ...
        Preconditions.checkArgument(environment.keySet().stream().noneMatch(key -> key.contains("=")), "Illegal environment variable: name cannot contain '='");
        return environment.entrySet().stream().map(entry -> entry.getKey() + "=" + entry.getValue()).collect(ImmutableList.toImmutableList());
    }
    ...
    private static <E> Map<String, Map<?, ?>> setToMap(@Nullable Set<E> set, Function<E, String> keyMapper) {
        ...
        return set.stream().collect(ImmutableSortedMap.toImmutableSortedMap(String::compareTo, keyMapper, ignored -> Collections.emptyMap()));
    }
