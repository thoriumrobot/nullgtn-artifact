...
class ActiveTracker implements LeakInstanceTracker {
    ...
    ActiveTracker(ULeakEventReporter reporter, ULeakConfiguration configuration, PublishSubject<Throwable> errorRelay) {
        this(reporter, configuration, new GcCallWrapper(), new RefGeneratorFactory(), new Random(), errorRelay);
    }

    @VisibleForTesting
    ActiveTracker(ULeakEventReporter reporter, ULeakConfiguration configuration, GcCallWrapper gcCallWrapper, RefGeneratorFactory refGeneratorFactory, Random random, PublishSubject<Throwable> errorRelay) {
        this.errorRelay = errorRelay;
        this.reporter = reporter;
        long leakCheckFrequencyMillis = configuration.leakCheckFrequencyMs;
        observableInterval = new ObservableInterval(leakCheckFrequencyMillis, leakCheckFrequencyMillis, TimeUnit.MILLISECONDS, Schedulers.io());
    ...
    public void flushAllQueuedReferences(ConcurrentLinkedQueue<ULeak.QueuedLeakEvent> events) {
        ...
        synchronized (trackedReferences) {
            while ((queuedLeakEvent = events.poll()) != null) {
                ...
                LeakTracker tracker = trackedReferences.getOrCreate(object.getClass(), SINGLETON, queuedLeakEvent.className, refGeneratorFactory);
                if (tracker == null) {
                    continue;
                }
    ...
    @Override
    public void registerNewReference(Object object, String className, final long timeCreatedMs, final LeakType leakType) {
    ...
    private void startTracking() {
        synchronized (observableInterval) {
            ...
            disposable = observableInterval.subscribe(new Consumer<Long>() {

                @Override
                public void accept(Long aLong) throws Exception {
                    boolean useExplicitGcThisRound = configuration.shouldUseExplicitGc() && random.nextDouble() < configuration.explicitGCProbability;
    ...
    private ULeakEvent createReportingEvent(final LeakTracker tracker) {
        ...
        reporterEvent.addDimension(GC_TYPE_DIMENSION_KEY_NAME, explicitGcCalledLastIteration.get() ? ViolationType.EXPLICIT_GC_VIOLATION.name() : ViolationType.IMPLICIT_GC_VIOLATION.name());
        reporterEvent.addDimension(LEAK_TYPE_KEY_NAME, tracker.getLeakType().name());
        reporterEvent.addMetric(INSTANCES_COUNT_METRIC_KEY_NAME, tracker.getCurrentNumInMemoryInstances());
        reporterEvent.addMetric(LAST_LEAK_CREATED_TIME_MS, tracker.getLatestReferenceRegistrationTimeMs());
    ...
    enum ViolationType {
        ...
        EXPLICIT_GC_VIOLATION, IMPLICIT_GC_VIOLATION
    }
