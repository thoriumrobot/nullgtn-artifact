...
public class PlantDetailActivity extends BaseColdsnapActivity<PlantDetailPresenter> implements PlantDetailMvpView {
    ...
    @BindView(R.id.pdv_plant_detail)
    PlantDetailView plantDetailView;

    @BindView(R.id.cl_plant_detail)
    CoordinatorLayout clPlantDetail;

    @BindView(R.id.ppiv_plant_profile_view)
    PlantProfileImageView ppivPlantProfileImageView;
    ...
    public static Intent newIntent(Context context, @Nullable UUID plantUUID, boolean isNewPlant) {
        return new Intent(context, PlantDetailActivity.class).putExtra(PLANT_UUID, plantUUID).putExtra(IS_NEW_PLANT, isNewPlant);
    }

    @Override
    protected void inject() {
        ((ColdSnapApplication) getApplicationContext()).getInjector().plantDetailSubcomponent(new PlantDetailModule(this)).inject(this);
    }
    ...
    public Observable<PlantDetailViewUpdate> plantDetailSaves() {
        return RxMenuItem.clicks(toolbar.getMenu().findItem(R.id.menu_item_save_plant)).map(i -> plantDetailView.getViewModel()).map(vm -> PlantDetailViewUpdate.builder().name(vm.getName()).scientificName(vm.getScientificName()).tempVal(vm.getTemperaturePickerViewModel().getValue()).build());
    }
    ...
    public void takePhoto(String fileName) {
        ...
        if (takePictureIntent.resolveActivity(getPackageManager()) != null) {
            ...
            if (photoFile != null) {
                Uri photoURI = FileProvider.getUriForFile(this, "com.yoxjames.coldsnap.provider", photoFile);
