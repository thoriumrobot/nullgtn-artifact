...
public final class HillClimberWindowTinyLfuPolicy implements Policy {
    ...
    public HillClimberWindowTinyLfuPolicy(HillClimberType strategy, double percentMain, HillClimberWindowTinyLfuSettings settings) {
        String name = String.format("sketch.HillClimberWindowTinyLfu (%s %.0f%%)", strategy.name().toLowerCase(US), 100 * (1.0 - percentMain));
    ...
    private void printSegmentSizes() {
        if (debug) {
            System.out.printf("maxWindow=%d, maxProtected=%d, percentWindow=%.1f", maxWindow, maxProtected, (100.0 * maxWindow) / maximumSize);
        }
    }
    ...
    static final class HillClimberWindowTinyLfuSettings extends BasicSettings {
        ...
        public Set<HillClimberType> strategy() {
            return config().getStringList("hill-climber-window-tiny-lfu.strategy").stream().map(strategy -> strategy.replace('-', '_').toUpperCase(US)).map(HillClimberType::valueOf).collect(toSet());
        }
    }
}
