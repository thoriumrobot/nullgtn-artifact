...
public final class FeedbackWindowTinyLfuPolicy implements Policy {
    ...
    public FeedbackWindowTinyLfuPolicy(double percentMain, FeedbackWindowTinyLfuSettings settings) {
        String name = String.format("sketch.FeedbackWindowTinyLfu " + "(%.0f%%)", 100 * (1.0d - percentMain));
        ...
        feedback = settings.membershipFilter().create(sampleSize, settings.adaptiveFpp(), settings.config());
    ...
    void printSegmentSizes() {
        if (debug) {
            System.out.printf("maxEden=%d, maxProtected=%d, percentEden=%.1f%n", maxEden, maxProtected, (double) (100 * maxEden) / maximumSize);
        }
    }
