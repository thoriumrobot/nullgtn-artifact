...
public final class OhcPolicy implements Policy {
    ...
    public OhcPolicy(OhcSettings settings, Eviction policy) {
        cache = OHCacheBuilder.<Long, Long>newBuilder().capacity(ENTRY_SIZE * settings.maximumSize()).edenSize(settings.percentEden()).valueSerializer(longSerializer).keySerializer(longSerializer).eviction(policy).build();
        policyStats = new PolicyStats(String.format("product.OHC (%s)", (policy == Eviction.LRU) ? "Lru" : "W-TinyLfu"));
    }
