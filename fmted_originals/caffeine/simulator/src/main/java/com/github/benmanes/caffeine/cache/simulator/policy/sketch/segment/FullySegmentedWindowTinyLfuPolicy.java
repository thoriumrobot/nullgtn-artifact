...
public final class FullySegmentedWindowTinyLfuPolicy implements Policy {
    ...
    public FullySegmentedWindowTinyLfuPolicy(double percentMain, FullySegmentedWindowTinyLfuSettings settings) {
        String name = String.format("sketch.FullySegmentedWindowTinyLfu (%.0f%%)", 100 * (1.0d - percentMain));
    ...
    public void finished() {
        long edenProbationSize = data.values().stream().filter(n -> n.status == Status.EDEN_PROBATION).count();
        long edenProtectedSize = data.values().stream().filter(n -> n.status == Status.EDEN_PROTECTED).count();
    ...
    enum Status {
        ...
        EDEN_PROBATION, EDEN_PROTECTED, MAIN_PROBATION, MAIN_PROTECTED
    }
