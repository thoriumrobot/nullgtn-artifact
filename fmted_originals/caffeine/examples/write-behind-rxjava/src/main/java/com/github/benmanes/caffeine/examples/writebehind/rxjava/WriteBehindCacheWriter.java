...
public final class WriteBehindCacheWriter<K, V> implements CacheWriter<K, V> {
    ...
    private WriteBehindCacheWriter(Builder<K, V> builder) {
        ...
        subject.buffer(builder.bufferTimeNanos, TimeUnit.NANOSECONDS).map(entries -> entries.stream().collect(toMap(Entry::getKey, Entry::getValue, builder.coalescer))).subscribe(builder.writeAction::accept);
    }
