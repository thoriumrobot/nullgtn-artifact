...
abstract class LocalAsyncLoadingCache<K, V> implements LocalAsyncCache<K, V>, AsyncLoadingCache<K, V> {
    ...
    /* ---------------- Synchronous views -------------- */
    static final class LoadingCacheView<K, V> extends AbstractCacheView<K, V> implements LoadingCache<K, V> {
        ...
        public void refresh(K key) {
            ...
            if ((oldValueFuture == null) || (oldValueFuture.isDone() && oldValueFuture.isCompletedExceptionally())) {
                asyncCache.get(key, asyncCache.loader::asyncLoad, /* recordStats */
                false);
                return;
            ...
            oldValueFuture.thenAccept(oldValue -> {
                ...
                CompletableFuture<V> refreshFuture = (oldValue == null) ? asyncCache.loader.asyncLoad(key, asyncCache.cache().executor()) : asyncCache.loader.asyncReload(key, oldValue, asyncCache.cache().executor());
