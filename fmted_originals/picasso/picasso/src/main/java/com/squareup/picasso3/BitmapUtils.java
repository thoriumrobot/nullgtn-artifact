...
final class BitmapUtils {
    ...
    static void calculateInSampleSize(int reqWidth, int reqHeight, @NonNull BitmapFactory.Options options, Request request) {
        calculateInSampleSize(reqWidth, reqHeight, options.outWidth, options.outHeight, options, request);
    }

    static boolean shouldResize(boolean onlyScaleDown, int inWidth, int inHeight, int targetWidth, int targetHeight) {
    ...
    static Bitmap decodeStream(Source source, Request request) throws IOException {
        ...
        Bitmap bitmap = SDK_INT >= 28 ? decodeStreamP(request, bufferedSource) : decodeStreamPreP(request, bufferedSource);
        ...
        return ImageDecoder.decodeBitmap(imageSource, new ImageDecoder.OnHeaderDecodedListener() {
            ...
            public void onHeaderDecoded(@NonNull ImageDecoder imageDecoder, @NonNull ImageDecoder.ImageInfo imageInfo, @NonNull ImageDecoder.Source source) {
                if (request.hasSize()) {
                    ...
                    if (shouldResize(request.onlyScaleDown, size.getWidth(), size.getHeight(), request.targetWidth, request.targetHeight)) {
